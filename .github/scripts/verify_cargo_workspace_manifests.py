...
"""Verify that codex-rs crates follow workspace manifest policy.

This keeps Cargo-based checks aligned with the workspace policy by ensuring
each crate opts into `[lints] workspace = true`, follows the package naming
conventions, and does not introduce new workspace crate features while the
existing exceptions are being removed.
"""
