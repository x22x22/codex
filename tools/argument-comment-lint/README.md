...
`run-prebuilt-linter.sh` also makes sure the `rustup` shims stay ahead of any
direct toolchain `cargo` binary on `PATH`, because `cargo-dylint` needs a
`cargo` that can dispatch `+nightly-2025-09-18-<host>` correctly.
