...
static DEFAULT_CLIENT_FACTORY: OnceLock<fn() -> CodexHttpClient> = OnceLock::new();
...
#[error("default client factory already initialized")]
pub struct SetDefaultClientFactoryError;

pub fn set_default_client_factory(
    factory: fn() -> CodexHttpClient,
) -> Result<(), SetDefaultClientFactoryError> {
    DEFAULT_CLIENT_FACTORY
        .set(factory)
        .map_err(|_| SetDefaultClientFactoryError)
}
...
fn create_client() -> CodexHttpClient {
    DEFAULT_CLIENT_FACTORY.get().copied().map_or_else(
...
fn try_parse_error_message(text: &str) -> String {
    tracing::debug!("Parsing server error response: {text}");
