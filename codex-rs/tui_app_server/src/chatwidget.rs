...
impl ChatWidget {
    ...
    fn execute_slash_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Collab => {
                ...
                let Some(mask) = collaboration_modes::presets_for_tui(self.model_catalog.as_ref())
                    .into_iter()
            ...
            SlashCommand::Review => match SlashCommandInvocation::parse_args(
                ...
                Ok(args) => match Self::parse_review_request(&args) {
                    Ok(review_request) => {
                        self.submit_op(AppCommand::review(review_request));
                        QueueReplayControl::Stop
                    }
                    Err(message) => {
                        self.restore_invalid_inline_slash_command(cmd, args_message, message)
    ...
    pub(crate) fn resume_selection_draft(
        ...
        let mut args = vec![target_session.thread_id.to_string()];
        let Some(path) = target_session.path.as_ref() else {
            return SlashCommandInvocation::with_args(SlashCommand::Resume, args)
                .into_user_message();
        };
        let (path_flag, path_value) = if let Some(path) = path.to_str() {
        ...
        SlashCommandInvocation::with_args(SlashCommand::Resume, args).into_user_message()
    }
