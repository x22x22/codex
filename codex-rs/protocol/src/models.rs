...
- ["a", "b", "c"]"#
    ...
    fn serializes_local_image_outputs_as_array() -> Result<()> {
        ...
        assert!(matches!(
            items,
            [FunctionCallOutputContentItem::InputImage { image_url, detail: None }]
    ...
    #[test]
    fn local_image_output_missing_file_becomes_text_placeholder() -> Result<()> {
        let call_tool_result = CallToolResult {
            content: vec![serde_json::json!({
                "type": "local_image",
        ...
        assert!(matches!(
            items,
            [FunctionCallOutputContentItem::InputText { text }]
                if text.contains("could not read the local image")
        ));
