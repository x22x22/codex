...
use crate::config_types::PluginConfig;
use crate::plugins::PluginAnalyticsHook;
use crate::plugins::PluginConfigEdit;
use crate::plugins::PluginConfigPersister;
use crate::plugins::PluginLoadRequest;
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        chatgpt_base_url: request.chatgpt_base_url.clone(),
        account_id,
        chatgpt_user_id,
...
pub struct PluginsManager {
    ...
    analytics_hook: RwLock<Option<Arc<dyn PluginAnalyticsHook>>>,
}
...
impl PluginsManager {
    ...
    pub fn new_with_restriction_product(
        ...
        Self {
            ...
            restriction_product,
            analytics_hook: RwLock::new(None),
        }
    }

    pub fn set_analytics_hook(&self, analytics_hook: Arc<dyn PluginAnalyticsHook>) {
        let mut stored_client = match self.analytics_hook.write() {
            Ok(client_guard) => client_guard,
            Err(err) => err.into_inner(),
        };
        *stored_client = Some(analytics_hook);
    }
    ...
    pub fn plugins_for_request(&self, request: &PluginLoadRequest) -> PluginLoadOutcome {
        self.plugins_for_request_with_force_reload(request, /*force_reload*/ false)
    }

    pub fn plugins_for_request_with_force_reload(
        &self,
        ...
        let outcome = load_plugins_from_layer_stack(&request.config_layer_stack, &self.store);
    ...
    pub async fn featured_plugin_ids(
        ...
        let cache_key = featured_plugin_ids_cache_key(request, auth);
        if let Some(featured_plugin_ids) = self.cached_featured_plugin_ids(&cache_key) {
            return Ok(featured_plugin_ids);
        }
        let featured_plugin_ids = fetch_remote_featured_plugin_ids(request, auth).await?;
    ...
    pub async fn install_plugin(
        ...
        config_persister: &dyn PluginConfigPersister,
        ...
        self.install_resolved_plugin(resolved, config_persister)
            .await
    }

    pub async fn install_plugin_with_remote_sync(
        &self,
        remote_request: &PluginRemoteRequest,
        auth: Option<&CodexAuth>,
        request: PluginInstallRequest,
        config_persister: &dyn PluginConfigPersister,
    ) -> Result<PluginInstallOutcome, PluginInstallError> {
        ...
        // reconcile pass here.
        enable_remote_plugin(remote_request, auth, &plugin_id)
            .await
    ...
    async fn install_resolved_plugin(
        ...
        config_persister
            .enable_plugin(&result.plugin_id.as_key())
            .await?;

        let analytics_hook = match self.analytics_hook.read() {
            Ok(client) => client.clone(),
            Err(err) => err.into_inner().clone(),
        };
        if let Some(analytics_hook) = analytics_hook {
            analytics_hook.track_plugin_installed(plugin_telemetry_metadata_from_root(
                &result.plugin_id,
    ...
    pub async fn uninstall_plugin(
        ...
        self.uninstall_plugin_id(plugin_id, config_persister).await
    }
    ...
    pub async fn uninstall_plugin_with_remote_sync(
        ...
        // `plugin/list(forceRemoteSync=true)` to sync local state rather than doing an extra
        // reconcile pass here.
        uninstall_remote_plugin(remote_request, auth, &plugin_key)
            .await
    ...
    async fn uninstall_plugin_id(
        ...
        config_persister.clear_plugin(&plugin_id.as_key()).await?;
        ...
        if let Some(plugin_telemetry) = plugin_telemetry
            && let Some(analytics_hook) = analytics_hook
        {
            analytics_hook.track_plugin_uninstalled(plugin_telemetry);
        }
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let remote_plugins = fetch_remote_plugin_status(request, auth)
            .await
            .map_err(PluginRemoteSyncError::from)?;
        let configured_plugins = configured_plugins_from_stack(&request.config_layer_stack);
        ...
        ) in local_plugins
        {
            ...
            if remote_installed_plugin_names.contains(&plugin_name) {
                ...
                if current_enabled.is_some() {
                    config_edits.push(PluginConfigEdit::ClearPlugin {
                        plugin_id: plugin_key,
                    });
                }
            }
        ...
        let config_result = if config_edits.is_empty() {
            ...
            config_persister.apply_plugin_edits(&config_edits).await
        };
        self.clear_cache();
        config_result?;
    ...
    pub fn list_marketplaces(
        ...
        let (installed_plugins, enabled_plugins) = self.configured_plugin_states(request);
    ...
    pub fn read_plugin(
        &self,
        plugin_load_request: &PluginLoadRequest,
        plugin_read_request: &PluginReadRequest,
    ) -> Result<PluginReadOutcome, MarketplaceError> {
        if !plugin_load_request.plugins_enabled {
            return Err(MarketplaceError::PluginsDisabled);
        }

        let marketplace = load_marketplace(&plugin_read_request.marketplace_path)?;
        let marketplace_name = marketplace.name.clone();
        let plugin = marketplace
            .plugins
            .into_iter()
            .find(|plugin| plugin.name == plugin_read_request.plugin_name);
        let Some(plugin) = plugin else {
            return Err(MarketplaceError::PluginNotFound {
                plugin_name: plugin_read_request.plugin_name.clone(),
                marketplace_name,
            });
        };
        if !self.restriction_product_matches(plugin.policy.products.as_deref()) {
        ...
        let (installed_plugins, enabled_plugins) =
            self.configured_plugin_states(plugin_load_request);
        let source_path = match &plugin.source {
    ...
    pub fn maybe_start_plugin_startup_tasks(
        ...
        if request.plugins_enabled {
            let mut configured_curated_plugin_ids =
                configured_plugins_from_stack(&request.config_layer_stack)
                    .into_keys()
            ...
            tokio::spawn(async move {
                ...
                if let Err(err) = manager.featured_plugin_ids(&request, auth.as_ref()).await {
                    warn!(
                        error = %err,
