...
#[derive(Clone, Copy)]
pub(crate) enum RequestSpanName {
    AppServerRequest,
    ThreadStart,
}

pub(crate) fn request_span(
    span_name: RequestSpanName,
...
pub(crate) fn detached_request_span(
    ...
    attach_parent_context(&span, method, &request_id.request_id, parent_trace);
    span
}
...
fn build_request_span(
    ...
    match span_name {
        RequestSpanName::AppServerRequest => info_span!(
            "app_server.request",
            otel.kind = "server",
            otel.name = method,
            rpc.system = "jsonrpc",
        ...
        RequestSpanName::ThreadStart => info_span!(
            "thread/start",
            otel.kind = "server",
            otel.name = method,
            rpc.system = "jsonrpc",
