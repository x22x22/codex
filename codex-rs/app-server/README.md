...
- `config/read` — fetch the effective config on disk after resolving config layering. In `tools`, `web_search` accepts either a boolean toggle or an object with `enabled` plus nested Responses web search options.
