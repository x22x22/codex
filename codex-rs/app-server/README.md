...
- `config/read` — fetch the effective config on disk after resolving config layering. In `tools`, `web_search` is a boolean toggle and `web_search_config` carries nested Responses web search options.
