...
#[tokio::test]
async fn thread_resume_allows_running_unmaterialized_thread() -> Result<()> {
    ...
    // Resume should still succeed while the thread is running, even before the
    // first user message materializes rollout storage.
    let resume_id = mcp
