...
#[tokio::test]
async fn thread_archive_works_before_first_user_message() -> Result<()> {
    ...
    assert!(rollout_path.exists(), "fresh thread rollout should exist");
    assert!(
        find_thread_path_by_id_str(codex_home.path(), &thread.id)
            .await?
            .is_some(),
        "thread id should be discoverable immediately after startup"
    );
