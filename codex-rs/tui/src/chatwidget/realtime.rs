...
const REALTIME_CONVERSATION_PROMPT: &str = concat!(
    ...
    "If the user asks to do something next, later, after this, or once current work finishes, call codex right away so the work is actually queued. ",
    "Do not merely say that you will do it later. Use codex to queue it. ",
    "Set send_immediately=true only when you truly need to steer the current Codex turn right away. ",
    "For future work requests, prefer codex with the default queued behavior and do not set send_immediately=true. ",
