...
fn ensure_test_model_catalog(config: &mut Config) {
    ...
    let bundled_models_path =
        codex_utils_cargo_bin::find_resource!("../models.json").expect("bundled models.json");
    let bundled_models_contents =
        std::fs::read_to_string(bundled_models_path).expect("read bundled models.json");
    let bundled_models: ModelsResponse =
        serde_json::from_str(&bundled_models_contents).expect("bundled models.json");
