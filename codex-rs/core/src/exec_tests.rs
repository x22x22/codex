...
#[test]
fn full_buffer_capture_policy_disables_caps_and_timeouts() {
    assert_eq!(ExecCapturePolicy::FullBuffer.retained_bytes_cap(), None);
    assert_eq!(ExecCapturePolicy::FullBuffer.io_drain_timeout(), None);
...
#[test]
fn build_exec_request_preserves_capture_policy() -> Result<()> {
    ...
    assert_eq!(exec_request.capture_policy, ExecCapturePolicy::FullBuffer);
