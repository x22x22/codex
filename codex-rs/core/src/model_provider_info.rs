...
fn default_provider_auth_timeout_ms() -> u64 {
...
impl ModelProviderInfo {
    pub(crate) fn validate(&self) -> std::result::Result<(), String> {
        ...
        if auth.timeout_ms == 0 {
            return Err("provider auth.timeout_ms must be greater than zero".to_string());
        }
        if auth.refresh_interval_ms == 0 {
            return Err("provider auth.refresh_interval_ms must be greater than zero".to_string());
        }
