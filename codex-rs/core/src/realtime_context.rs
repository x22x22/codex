...
mod tests {
    ...
    #[test]
    fn current_thread_section_includes_last_two_turns() {
        let items = vec![
        ...
        assert!(!section.contains("first answer"));
    }
    ...
    fn current_thread_section_skips_contextual_user_messages() {
        let items = vec![
            ResponseItem::Message {
                ...
                role: "user".to_string(),
                content: vec![ContentItem::InputText {
                    text: ENVIRONMENT_CONTEXT_FRAGMENT.wrap("synthetic context".to_string()),
                }],
                end_turn: None,
                phase: None,
            },
        ...
        let section = build_current_thread_section(&items).expect("current thread section");
        assert!(!section.contains("synthetic context"));
        assert!(section.contains("real question"));
        assert!(section.contains("real answer"));
    }
}
