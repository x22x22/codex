...
impl SessionConfiguration {
    ...
    async fn refresh_project_doc_instructions_for_cwd(&mut self) {
        let mut config = (*self.original_config_do_not_use).clone();
        config.cwd = self.cwd.clone();
        self.project_doc_instructions = build_project_doc_instructions_text(&config).await;
    }
...
impl Session {
    ...
    pub(crate) async fn update_settings(
        ...
        let (mut updated, previous_cwd) = {
            let state = self.state.lock().await;
            match state.session_configuration.apply(&updates) {
                Ok(updated) => (updated, state.session_configuration.cwd.clone()),
                Err(err) => {
        ...
        if previous_cwd != updated.cwd {
            updated.refresh_project_doc_instructions_for_cwd().await;
        }
    ...
    pub(crate) async fn new_turn_with_sub_id(
        ...
        let (
            ...
            let (mut next, previous_cwd, sandbox_policy_changed) = {
                ...
                match state.session_configuration.clone().apply(&updates) {
                    Ok(next) => {
                        ...
                        (next, previous_cwd, sandbox_policy_changed)
                    }
                    Err(err) => {
                        drop(state);
            ...
            if previous_cwd != next.cwd {
                next.refresh_project_doc_instructions_for_cwd().await;
            }
