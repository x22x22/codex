...
pub(crate) async fn read_text(
    ...
    let bytes = run_request(session, turn, &path_buf).await?;
    String::from_utf8(bytes).map_err(|error| SandboxedFsError::Operation {
        path: path_buf,
        error: FsError {
            kind: FsErrorKind::InvalidData,
            message: format!("file contents were not valid UTF-8: {error}"),
            raw_os_error: None,
        },
    })
}
