...
fn is_unconfigured_zsh_exec(program: &AbsolutePathBuf, shell_zsh_path: Option<&Path>) -> bool {
    let Some(shell_zsh_path) = shell_zsh_path else {
        return false;
    };
    program.as_path().file_name() == Some(OsStr::new("zsh")) && program.as_path() != shell_zsh_path
}
...
impl EscalationPolicy for CoreShellActionProvider {
    async fn determine_action(
        ...
        // intercepted exec targets host `zsh` instead of the configured
        // zsh-fork binary, force it through escalation so the executor can
        // rewrite the program path back to the configured shell.
        let force_zsh_fork_reexec =
            is_unconfigured_zsh_exec(program, self.session.services.shell_zsh_path.as_deref());
...
impl CoreShellCommandExecutor {
    fn rewrite_intercepted_program_for_zsh_fork(
        ...
        if !is_unconfigured_zsh_exec(program, Some(shell_zsh_path.as_path())) {
            return program.clone();
        }
