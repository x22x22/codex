...
impl ManagedFeatures {
    ...
    pub(crate) fn from_configured(
        ...
        let managed_features = Self::new(
            ConstrainedWithSource::new(Constrained::allow_any(normalized_features), source),
            pinned_features,
        );
        Ok(managed_features)
    }

    pub fn unconstrained(features: Features) -> Self {
        Self::new(
            ConstrainedWithSource::new(Constrained::allow_any(features), None),
            BTreeMap::new(),
        )
    }
