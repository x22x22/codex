...
impl ManagedFeatures {
    ...
    fn update(&mut self, update: impl FnOnce(&mut Features)) {
        ...
        update(&mut next);
        if let Err(err) = self.value.set(next) {
            panic!("managed feature constraints should remain satisfiable: {err}");
        }
    }
...
impl From<Features> for ManagedFeatures {
    fn from(features: Features) -> Self {
        match Self::from_configured(features, None) {
            Ok(features) => features,
            Err(err) => panic!("unconstrained features should always be constructible: {err}"),
        }
    }
}
