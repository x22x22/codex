...
pub enum ExecCapturePolicy {
    ...
    /// without the shell-oriented cap or timeout behavior.
    FullBuffer,
}
...
impl ExecCapturePolicy {
    ...
    fn io_drain_timeout(self) -> Option<Duration> {
        match self {
            Self::ShellTool => Some(Duration::from_millis(IO_DRAIN_TIMEOUT_MS)),
            Self::FullBuffer => None,
        }
    }
...
async fn exec(
    ...
    if sandbox == SandboxType::WindowsRestrictedToken {
        ...
        return exec_windows_sandbox(params, params.capture_policy, sandbox_policy).await;
    }
    let ExecParams {
        command,
        cwd,
...
async fn consume_output(
    ...
    let (exit_status, timed_out) = match capture_policy.uses_expiration() {
        true => {
            tokio::select! {
