...
async fn caches_command_output_until_refreshed() {
    ...
    let source = request_auth_source(&provider).expect("request auth source");

    let first = crate::auth::ExternalAuthRefresher::resolve(source.as_ref())
        .await
        .unwrap()
        .map(|tokens| tokens.access_token);
    let second = crate::auth::ExternalAuthRefresher::resolve(source.as_ref())
        .await
        .unwrap()
        .map(|tokens| tokens.access_token);
    let changed = crate::auth::ExternalAuthRefresher::refresh_after_unauthorized(
        source.as_ref(),
        crate::auth::ExternalAuthRefreshContext {
    ...
    .await
    .unwrap();
    let refreshed = crate::auth::ExternalAuthRefresher::resolve(source.as_ref())
        .await
        .unwrap()
        .map(|tokens| tokens.access_token);
