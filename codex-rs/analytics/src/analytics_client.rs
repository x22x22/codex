            ...
            while let Some(job) = receiver.recv().await {
                match job {
                    ...
                    TrackEventsJob::TurnEvent(job) => {
                        send_track_turn_event(&auth_manager, &base_url, job).await;
                    }
                    TrackEventsJob::TurnSteer(job) => {
                        send_track_turn_steer(&auth_manager, &base_url, job).await;
                    }
...
async fn send_track_turn_steer(auth_manager: &AuthManager, base_url: &str, job: TrackTurnSteerJob) {
