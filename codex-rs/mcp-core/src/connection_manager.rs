...
use crate::auth::McpAuthStatusEntry;
...
pub fn codex_apps_tools_cache_key_from_token_data(
...
fn is_connector_id_allowed(connector_id: &str) -> bool {
    ...
    const ORIGINATOR_ENV_VAR: &str = "OPENAI_ORIGINATOR";
    const FIRST_PARTY_CHAT_ORIGINATOR: &str = "chatgpt";

    let originator = env::var(ORIGINATOR_ENV_VAR).unwrap_or_default();
    let disallowed_connector_ids = if originator == FIRST_PARTY_CHAT_ORIGINATOR {
